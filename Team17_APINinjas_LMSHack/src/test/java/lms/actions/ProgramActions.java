...
public static List<RequestSpecification> getPostPutProgramRequestsDD(String url, String token, String type) throws IOException {
	 ...
	 List<Map<String, String>> testData = excelReader.readTestDataFromExcel(FileNameConstants.ProgramEXCEL_TEST_DATA, "ETEPositiveTestData-Program");
...
public static List<RequestSpecification> getPostProgramRequestsDDNegative(String url, String token) throws IOException {
	...
	ExcelReader excelReader = new ExcelReader();
	 List<Map<String, String>> testData = excelReader.readTestDataFromExcel(FileNameConstants.ProgramEXCEL_TEST_DATA, "PostPutTestData-Program");
       List<RequestSpecification> requestSpecifications = new ArrayList<>();
       //int columnCount = 0;
...
public static RequestSpecification getPostPutProgramRequestNoAuth(String url) throws JsonProcessingException{
...
public static RequestSpecification getGetDeleteProgramRequestNoAuth(String url) throws JsonProcessingException {
	request =  
			RestAssured
				.given()
					.contentType(ContentType.JSON)						
	...
	for (RequestSpecification request : requests) {
		...
		responses.add(response);
		 ...
		 if (response.path("programId") != null) {
            ...
            System.out.println("Created Program Id is: "+  Env_Var.programID );
            System.out.println("Created Program Name is: "+ Env_Var.programName );
...
public static void setProgramDetails(Response response) {
    ...
    if (response.path("programId") != null) {
        ...
        System.out.println("Created Program Id is: "+ programID );
        System.out.println("Created Program Name is: "+ programName );
        System.out.println("Created Envi Program Id is: "+  Env_Var.programID );
    }  
}
...
public static List<Response> getGetProgramResponsesDD(String by, String token, String url ) throws IOException {
	...
	List<Response> responses = new ArrayList<>();
	ExcelReader excelReader = new ExcelReader();
	List<Map<String, String>> testData = excelReader.readTestDataFromExcel(FileNameConstants.ProgramEXCEL_TEST_DATA, "GetDeleteTestData-Program");
   		   ...
   		   if( by.toLowerCase()=="programid") {
   			...
   			progID = row.get("ProgramID"); // Extract fields from each row  
   			   if (!progID.contains(".")) {
   				   ...
   				   int programid = Integer.parseInt(progID.split("\\.")[0]);
   				progID = Integer.toString(programid); 
   				   System.out.println("String Program Id from excel is: "+ progID);
   				   response = request.when().get("/{progID}", progID);            	
   			   	}           
   		   }
...
public static List<Response> getDeleteProgramResponsesDD(String token, String url ) throws IOException {
	...
	for (Map<String, String> row : testData) {    	
    	...
    	progID = row.get("ProgramID"); // Extract fields from each row  
   			   if (!progID.contains(".")) {
   				   System.out.println("String Batch Id from excel is: "+ progID);
...
public static Response ProgramResponseInvalidUrl(RequestSpecification request, String type) {
